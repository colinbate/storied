<script lang="ts">
	...
	const externalSourceLabel = $derived(data.series.hardcoverUrl ? 'Hardcover' : 'Goodreads');
	const externalSourceUrl = $derived(data.series.hardcoverUrl ?? data.series.goodreadsUrl);
...
<svelte:head>
	<title>{pageTitle(data.series.title)}</title>
</svelte:head>

<div class="space-y-6">
	<LibraryNav />
	<LibraryBreadcrumb section="Series" sectionHref="/series" title={data.series.title} />
