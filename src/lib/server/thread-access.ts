...
import { groupMemberships, groups, threads } from '$lib/server/db/schema';
...
export function threadAccessCondition(db: ORM, viewer: ThreadViewer) {
	if (viewer.canModerate) return sql`1 = 1`;
	if (!viewer.userId) {
		return and(eq(threads.visibility, 'public'), isNull(threads.audienceGroupId));
	}
 ...
 * The returned fragment has two bind parameters: moderator flag, then user id.
 */
...
export function threadAccessBindings(viewer: ThreadViewer): [number, string] {
	return [viewer.canModerate ? 1 : 0, viewer.userId ?? ''];
}
